...
class BaseDataset(torchdata.Dataset):
    ...
    def _load_audio(self, path, center_timestamp, nearest_resample=False):
        ...
        if self.split == 'train':
            ...
            audio_mix = np.abs(audio.min())
            ratio = np.max(audio_max, audio_min)
            audio /= ratio
        """
        #if self.init_clip:
        #    audio[audio > 1.] = 1.
    ...
    def _mix_n_and_stft(self, audios):
        ...
        #order = [0, 1]
        #if mags[1].mean() > mags[0].mean():
        #    mags[1], mags[0] = mags[0], mags[1]
        #    order = [1, 0]

        mag_means = [mag.mean() for mag in mags]
        order = list(reversed(np.argsort(np.array(mag_means))))
        ...
        return amp_mix.unsqueeze(0), mags, phase_mix.unsqueeze(0), order
