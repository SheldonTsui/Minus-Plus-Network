...
if __name__ == '__main__':
    ...
    for audio_path in audio_files:
        ...
        frame_name = 'folder_' + file_id
        frame_path = audio_path.replace(args.root_audio, args.root_frame).replace(file_id + '.wav', frame_name)
        frame_files = glob.glob(frame_path + '/*.jpg')
        if len(frame_files) > args.fps * 20: # ensure the video at least 20 seconds
